...
(function(){
    ...
    var getDamage = function(stalker, duration){
        ...
        if (stalker.punish > 3){
            var punishHits = abilities.PUNISH.hits;
            var exposeDuration = punishHits * Simulator.ABILITIES.PUNISH.T4_EXPOSE_DURATION;
            exposeDuration = Math.min(exposeDuration, duration);
            var uptime = exposeDuration / duration;

            abilities.forEach(function(ability){
                if(ability.damageType === 'phys'){
                    ability.rawDamage *= (1 + (Simulator.ABILITIES.PUNISH.T4_EXPOSE * uptime));
                    ability.damage *= (1 + (Simulator.ABILITIES.PUNISH.T4_EXPOSE * uptime));
                }
            });
        }
